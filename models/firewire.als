 ...
 *
 * The network is assumed to consist of a collection of Node1s connected by
 * links. Each link between a pair of Node1s is matched by a link in the other
 ...
 * subset of the links as parent links (each pointing from a Node1 to its parent),
 * and by marking a single Node1 as the root.
 *
 * The algorithm, described in detail elsewhere [DG+01], works briefly as
 * follows. When a Node1 detects that all of its incoming links (or all but one)
 ...
 * the outgoing link has been marked or not. Leaf Node1s (with only one incoming
 * link) may thus initiate the algorithm by sending requests to their adjacent
 * Node1s. Performing this action changes a Node1's status from {\em waiting} to
 * {\em active}. A Node1 that is still waiting, and which receives a message on a
 * link, may label that link a parent link. Once active, a Node1 that receives an
 * acknowledgment on a link may also label the link, but if it receives a request,
 * instead changes its Node1 status to {\em contending}. The resolving of
 * contentions is modelled simplistically by a single action that arbitrarily
 * labels one of the two links a pair of contending Node1s. Finally, a Node1 all of
 ...
 *
 * \item {\em Node1} represents the Node1s of the network. The relations {\em to}
 * and {\em from} associate each Node1 with a set of incoming and outgoing links
 * respectively.
 *
 * \item {\em Link} represents the links. The relations {\em target} and {\em
 * source} map a link to its end Node1s; {\em reverse} maps a link to its dual. The
 * facts in the signatures {\em Node1} and {\em Link} ensure that all these
 * relations are consistent with one another: that the incoming links of a Node1
 * are those whose target is that Node1, etc.
 *
 ...
 * the Node1s into one of four statuses: {\em waiting} to participate, {\em active}
 ...
 * Topology} says that there is some partial function on Node1s and some root such
 * that (1) every Node1 is reachable from the root ({\tt *r} being the reflexive
 ...
 * relation on Node1s); and (3) the relation obtained by following the {\em source}
 * relation backwards (from a Node1 to the link for which it is a source), and then
 ...
 * constrains with respect to the Node1 at which the operation occurs. The
 * subformulas, from first to last, say that the Node1 belongs to the waiting set
 ...
 * changes to Node1 status except at this Node1; that a message is queued onto each
 ...
 * there is at most one Node1 that has been elected; that there is some state in
 * which a Node1 has been elected; and that no queue overflows.
 *
 ...
 * an execution involving 2 Node1s, 4 links, 4 queues and a trace of 6 states. The
 ...
 * various topology bounds. For 2 Node1s and 2 links, for example, there are no
 ...
 * these with overflow. For 3 Node1s and 6 links, a trace length of 8 suffices.
 *
 ...
 * to verify the properties for all topologies involving the given number of Node1s
...
sig Node1 {to, from: set Link} {
  to = {x: Link | x.target = this}
  from = {x: Link | x.source = this}
  }

sig Link {target, source: Node1, reverse: Link} {
  reverse.@source = target
  reverse.@target = source
  }

/**
 * at most one link between a pair of Node1s in a given direction
 */
 ...
 */
fact Topology {
some tree: Node1 lone -> Node1, root: Node1 {
  Node1 in root.*tree
  no ^tree & iden & Node1->Node1
...
sig State {
  disj waiting, active, contending, elected: set Node1,
  parentLinks: set Link,
  queue: Link -> one Queue,
  op: Op -- the operation that produced the state
  } {
  waiting + active + contending + elected = Node1
}
...
pred NoChangeExceptAt [s, s': State, Node1s: set Node1] {
  let ns = Node1 - Node1s {
