...
pred less_than [from, i,j: Id] {
  let next' = Id<:next - (Id->from) | j in i.^next'  // if from=j, returns true if # Node1s > 1
  }
...
/**
 * Node1 n's next Node1 is defined to be the m where n's finger table maps the id
...
pred NextCorrect [s: State] {
  all n: s.active {
    -- no intervening Node1 (ie, close enough)
    no n': s.active - n | less_than [n.id, n'.id, n.(s.data).next.id]
    -- can reach all other active Node1s (ie, far enough)
    -- need this because can't rule out case of next being Node1 itself (because of 1-Node1 ring)
...
pred NextCorrect' [s: State] {
-- next seems to be correct for 1,2,3 Node1s
...
assert Same2 {all s: State | s.active = Node1 => (NextCorrect'[s] => NextCorrect[s])}
...
/**
 * says that finger entry maps an id to a Node1 so that there are no intervening Node1s
 * between the id and the Node1
 */
...
pred FingersCorrect' [s: State] {
  all n: s.active | let nd = (s.data)[n] | all start: Node1.~(nd.finger) {
    nd.finger[start] in s.active &&
...
pred ShowMeFC {
  all s : State | s.active = Node1 && FingersCorrect[s]
}
...
pred CPFBody [s: State, n: Node1, nd: Node1Data, i: Id, cpf: Node1] {
  no n': (nd.finger[Id] + n) - cpf | less_than [cpf.id, n'.id, i]
  cpf in nd.finger[Id] + n
  cpf.id != i || # s.active = 1
  }
pred ClosestPrecedingFinger[s: State] {
  all n: s.active | let nd = n.(s.data) |
    all i: Id |
    some cpf: Node1 | CPFBody [s,n,nd,i,cpf] => CPFBody [s,n,nd,i,nd.closest_preceding_finger[i]]
  }
...
assert SameCPF2 {
  all s: State | ((s.active = Node1 && FingersCorrect[s] && ClosestPrecedingFinger' [s])
...
pred ShowMeCPF  {
  all s : State | s.active = Node1 && FingersCorrect[s] &&
        ...
        ClosestPrecedingFinger[s]
  //all s : State | all nd : s.active.s.data | nd.finger[Id] = Node1
  # Node1 = 2
  # State = 1
}
...
-- problem : could return Node1 that's inactive ???
...
assert SameFP {all s: State | FingersCorrect[s] && s.active = Node1
  => (FindPredecessor [s] iff FindPredecessor' [s])}
assert SameFP1 {
  all s: State | FingersCorrect[s] && s.active = Node1
...
// only active Node1s if FingersCorrect.
...
//run ShowMe1Node1 for 2 but 1 State, 1 Node1
-- does the expected correct thing for 1 Node1.
...
pred ShowMe2  {
  ...
  all s : State | NextCorrect[s] && FingersCorrect[s]
        State.active = Node1
  //all n: Node1Data | one n.finger[Id]
}
...
check FindSuccessorWorks for 4 but 1 State, 3 Node1, 3 Node1Data expect 0
